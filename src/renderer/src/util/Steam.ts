...
import { map, mapSeries, only } from "./asyncpromise";
...
class Steam implements IGameStore {
  ...
  constructor() {
    if (process.platform === "win32") {
      ...
      this.mBaseFolder = Promise.resolve(linuxPath);
    }
  }
  ...
  public identifyGame(
    ...
    return Promise.resolve(fallback(gamePath)).then((fbResult: boolean) => {
      if (fbResult !== custom) {
  ...
  private parseManifests(): Promise<ISteamEntry[]> {
    return this.resolveSteamPaths().then((steamPaths: string[]) =>
      mapSeries(steamPaths, (steamPath) => {
        ...
        return Promise.resolve(fsOG.readdir(steamAppsPath))
          .then((names) => {
            ...
            return map(filtered, (name: string) =>
