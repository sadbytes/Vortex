...
function updateLatestFileAttributes(
  dispatch: Redux.Dispatch<any>,
  gameId: string,
  mod: IMod,
  file: IFileInfo,
...
function updateFileAttributes(
  ...
  if (fileUpdates.length === 0) {
    ...
    if (notOld.length === 1 && notOld[0].file_id !== fileId) {
      ...
      const potentialCandidates = files.file_updates.filter(
        (up) => !isFileDeleted(up.new_file_id) && up.old_file_id === fileId,
      );
  ...
  if (changelog.length > 0) {
    update(dispatch, gameId, mod, "newestChangelog", {
      format: "html",
      content: changelog,
    });
  } else {
