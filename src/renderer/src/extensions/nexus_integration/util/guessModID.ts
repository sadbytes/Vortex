...
export function queryLookupResult(
  ...
  let text =
    "There are multiple potential matches. This usually happens when the same file " +
...
export function queryResetSource(api: IExtensionApi, gameId: string, mod: IMod) {
  ...
  return api
    .showDialog(
      ...
      {
        text:
          "This mod wasn't found on the Nexus Mods servers, maybe it was modified? " +
      ...
      [{ label: "Cancel" }, { label: "Reset source" }],
    )
    .then((result) => {
...
export function fillNexusIdByMD5(
  api: IExtensionApi,
  gameId: string,
  mod: IMod,
  fileName: string,
            ...
            try {
              ...
              if (mod.state === "installed") {
                ...
                if (isNewestVersion) {
                  ...
                  const versionRgx = new RegExp(
                    `${nxmUrl.modId}-(.*?)-\\d{10,}\\.(zip|rar|7z|tar|gz)$`,
                    "i",
                  );
                  ...
                  if (match) {
                    ...
                    const t = api.translate;
                    const question = await api.showDialog(
                      "question",
                      "Mod version",
                      {
                        bbcode: t(
                          'The version of this mod ("{{modName}}") is "{{version}}". Is this correct?[br][/br][br][/br]' +
