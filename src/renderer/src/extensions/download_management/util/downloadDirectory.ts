...
import { each, only } from "../../../util/asyncpromise";
...
export function writeDownloadsTag(api: IExtensionApi, tagPath: string): Promise<void> {
  ...
  return writeTag().catch(
    only({ code: "EISDIR" }, (err) => {
      ...
      return api
        .showDialog(
          ...
          {
            text:
              "Vortex expected the below filepath to lead to a file but found " +
        ...
        .catch(only({ code: "ENOENT" }, (remErr) => Promise.resolve()))
        .then(() => writeTag())
...
function removeDownloadsMetadata(api: IExtensionApi): Promise<void> {
  ...
  return each(Object.keys(downloads), (dlId) => {
    api.store.dispatch(removeDownload(dlId));
    return Promise.resolve();
  }).then(() => Promise.resolve());
}

function queryDownloadFolderInvalid(
  api: IExtensionApi,
  err: Error,
  dirExists: boolean,
  ...
  if (dirExists) {
    ...
    return api.showDialog(
      "error",
      "Downloads Folder invalid",
      {
        bbcode:
          'Your downloads folder "{{path}}" is not marked correctly. This may be ok ' +
  ...
  return api.showDialog(
    ...
    {
      text:
        'Your downloads folder "{{path}}" is missing. This might happen because you ' +
      ...
      if (tag.instance !== state.app.instanceId) {
        return api
          .showDialog(
            ...
            {
              text:
                "This is a downloads folder but it appears to belong to a different Vortex " +
    ...
    .catch(() => {
      return api
        .showDialog(
          ...
          {
            text:
              "This directory is not marked as a downloads folder. " +
          ...
          return removeDownloadsMetadata(api)
            ...
            .catch(() => {
              api.showDialog(
                ...
                {
                  bbcode:
                    "The downloads folder could not be created. " +
