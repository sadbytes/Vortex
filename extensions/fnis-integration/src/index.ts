...
function checkFailedResult(
  ...
  const long = {
    missing: t(
      "You have configured Vortex to run FNIS automatically but it's not installed for this game. " +
    ...
    outdated: t(
      "You have configured Vortex to run FNIS automatically but the installed version of FNIS is " +
  ...
  context.registerToDo(
    ...
    (props: IFNISProps) => {
      ...
      context.api.events.emit(
        ...
        "Dashboard",
        `FNIS ${props.enabled ? "ON" : "OFF"}`,
      );
    },
    (props: IFNISProps) => isSupported(props.gameMode),
    (t: TranslationFunction, props: IFNISProps) => (props.enabled ? t("Yes") : t("No")),
    undefined,
  );
  ...
  context.registerAction(
    ...
    () => {
      ...
      readFNISPatches(context.api, profile)
        .then((patches) => {
          return context.api.showDialog(
            "question",
            "Select patches for this profile",
            {
              text:
    ...
    if (tool !== undefined) {
      ...
      return fs
        ...
        .then(() => {
          ...
          log("debug", "fnis tool setup", {
            path: tool.path,
            version: versionStr || "missing",
          });
  ...
  context.once(() => {
    ...
    (context.api as any).onAsync("will-deploy", (profileId: string, deployment: IDeployment) => {
      ...
      if (util.getSafe(state, ["settings", "fnis", "autoRun"], false)) {
        ...
        if (
          util.getSafe(state, ["mods", modId], undefined) !== undefined &&
          ...
          return calcChecksum(discovery.path, deployment)
            ...
            .then(() => {
              ...
              dependentMods.forEach((refId) => {
                ...
                store.dispatch(
                  actions.addModRule(profile.gameId, modId, {
                    type: "after",
                    reference: { id: refId },
                  }),
                );
