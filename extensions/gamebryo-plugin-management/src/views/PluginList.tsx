...
class PluginList extends ComponentEx<IProps, IComponentState> {
  ...
  private renderCleaningData(dat: PluginCleaningData) {
    ...
    if (dat.deletedNavmeshCount > 0) {
      things.push(
        t("{{count}} deleted navmesh", {
          ns: NAMESPACE,
          count: dat.deletedNavmeshCount,
        }),
      );
    }
    if (dat.deletedReferenceCount > 0) {
      things.push(
        t("{{count}} deleted reference", {
          ns: NAMESPACE,
          count: dat.deletedReferenceCount,
        }),
      );
    }
          ...
          new Promise<void>((resolve, reject) => {
            ...
            if (pluginNames.length > 0) {
              this.context.api.events.emit(
                ...
                this.props.gameMode,
                pluginNames,
                (resolved: { [name: string]: IPluginLoot }) => {
  ...
  private modIndices(pluginObjects: IPluginCombined[]): {
    [pluginId: string]: {
      modIndex: number;
      eslIndex?: number;
      mediumIndex?: number;
  ...
  private applyLoadOrder(loadOrder: { [pluginId: string]: ILoadOrder }) {
    ...
    Object.keys(modIndices).forEach((pluginId) => {
      ...
      updateSet[pluginId].mediumIndex = {
        $set: modIndices[pluginId].mediumIndex,
      };
    });
  ...
  private prepareMessage(
    input: string | Array<{ text: string; language: string }>,
    plugin: IPluginCombined,
  ) {
          ...
          <ListGroupItem key={idx}>
            <Alert bsStyle={this.translateLootMessageType(msg.type)}>
              {(ReactMarkdown as any)({
                children: this.prepareMessage(msg.content, plugin),
              })}
            </Alert>
          </ListGroupItem>
        ))}
  ...
  private sortByLoadOrder = (
    native: { [id: string]: number },
    lhs: IPluginCombined,
    rhs: IPluginCombined,
  ) => {
    if (this.installedNative !== undefined) {
      const lhsLO = lhs.isNative ? native[lhs.id] : (lhs.loadOrder | 0) + 1000;
      const rhsLO = rhs.isNative ? native[rhs.id] : (rhs.loadOrder | 0) + 1000;
      return lhsLO - rhsLO;
    } else {
  ...
  private makeAttributes(): Array<types.ITableAttribute<IPluginCombined>> {
    return [
      ...
      },
      {
        ...
        customRenderer: (plugin: IPluginCombined, detail: boolean, t: TranslationFunction) => {
          ...
          return (
            <ListGroup>
              {flags.map((flag) => (
                <ListGroupItem key={`flags-${flag}`}>{t(flag)}</ListGroupItem>
              ))}
            </ListGroup>
          );
        },
      ...
      },
      {
        ...
        name: "Load Order",
        description:
          "The order in which plugins are loaded. " +
      ...
      },
      {
        ...
        name: "Mod Index",
        description:
          "The Mod index is the first two hexadecimal digits of all ids " +
      ...
      },
      {
        ...
        customRenderer: (plugins) => {
          ...
          if (!Array.isArray(plugins)) {
            plugins = plugins === undefined ? [] : [plugins];
          }
          return (
            <GroupSelect
              t={this.props.t}
      ...
      },
      {
        ...
        name: "Light",
        description:
          "A light plugin doesn't occupy a regular load order slot. " +
...
export default withTranslation(["common", NAMESPACE])(
  connect<IConnectedProps, IActionProps, IBaseProps>(
    mapStateToProps,
    mapDispatchToProps,
  )(PluginList) as any,
