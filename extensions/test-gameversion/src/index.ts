...
function isCompatible(gameId: string, mod: types.IMod, version: string): boolean {
  ...
  if (
    mod.attributes.minGameVersion !== undefined &&
    versionCompare(gameId, mod.attributes.minGameVersion, version) > 0
  ) {
    return false;
  }
  if (
    mod.attributes.maxGameVersion !== undefined &&
...
async function testGameVersions(api: types.IExtensionApi): Promise<types.ITestResult> {
  ...
  const incompatible = Object.keys(mods || {}).filter(
    (modId) => !isCompatible(gameMode, mods[modId], currentGameVersion),
  );
  ...
  const previousGameVersion = util.getSafe(
    state.persistent.gameMode,
    ["versions", gameMode],
    undefined,
  );
