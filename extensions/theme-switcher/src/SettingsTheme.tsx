...
type IProps = Omit<ISettingsThemeProps, "readThemes"> & {
  themes: string[];
} & IConnectedProps &
  IActionProps;
...
class SettingsTheme extends ComponentEx<IProps, IComponentState> {
  ...
  public UNSAFE_componentWillMount() {
    this.nextState.availableFonts = Array.from(
      new Set<string>(["Inter", "Roboto", "BebasNeue", "Montserrat"]),
    );
  ...
  public render(): JSX.Element {
    ...
    const canClone = themes.some((iter) => this.props.locationToName(iter) === currentTheme);
    return (
        ...
        <form>
          ...
          <FormGroup controlId="themeSelect">
            ...
            <InputGroup style={{ width: 300 }}>
              ...
              <InputGroup.Button>
                <Button bsStyle="primary" onClick={this.onClone} disabled={!canClone}>
                  {t("Clone")}
                </Button>
                {editable ? (
                  <Button bsStyle="primary" onClick={this.remove}>
                    {t("Remove")}
                  </Button>
                ) : null}
              </InputGroup.Button>
...
) as React.ComponentClass<ISettingsThemeProps>;
