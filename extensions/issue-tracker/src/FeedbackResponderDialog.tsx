...
class FeedbackResponderDialog extends ComponentEx<IProps, IComponentState> {
  ...
  public render(): JSX.Element {
    ...
    return (
      <Modal id="feedback-responder-dialog" show={open} onHide={this.close}>
        ...
        <Modal.Footer>{this.renderFooter(maySend)}</Modal.Footer>
      </Modal>
    );
  }
      ...
      return (
        <FlexLayout type="row">
          ...
          <FlexLayout type="column">
            ...
            <FlexLayout.Fixed>{this.renderAttachButton()}</FlexLayout.Fixed>
          </FlexLayout>
        </FlexLayout>
      );
    } else {
  ...
  private select = (evt) => {
    ...
    const outstanding = outstandingIssues.find(
      (iss) => iss.issue.number.toString() === issueNumber,
    );
  ...
  private openLink = (evt) => {
    util.opn(evt.currentTarget.getAttribute("data-link")).catch((err) => null);
  };
  ...
  private renderGithubIssue = (issue: IGithubIssue, idx: number) => {
    ...
    const classes =
      currentIssue.number === issue.number
        ? ["responder-github-issue", "selected"]
        : ["responder-github-issue"];
    return (
      <ListGroupItem
        key={key}
      ...
      >
        <div className="issue-header">
          <FlexLayout type="row" style={{ justifyContent: "space-between" }}>
            ...
            <div>
              <a data-link={issue.html_url} onClick={this.openLink}>
                #{issue.number}
              </a>
            </div>
          </FlexLayout>
      ...
      <FlexLayout fill={false} type="row" className="feedback-controls">
        ...
        <FlexLayout.Fixed>
          <tooltip.Button
            ...
            disabled={sending || feedbackMessage.length === 0 || !valid}
          >
            {t("Submit Feedback")}
          </tooltip.Button>
        </FlexLayout.Fixed>
  ...
  private attach = (eventKey: any) => {
    switch (eventKey) {
      ...
      case "actions":
        ...
        break;
      case "session":
        this.attachState("session", "Vortex Session");
        break;
      case "settings":
  ...
  private openFileBrowser() {
    return this.context.api
      ...
      .then((selectedPath) =>
        fs.statAsync(selectedPath).then((stat) => {
  ...
  private attachActions(name: string) {
    tmpFile(
      ...
      (err, tmpPath: string, fd: number, cleanup: () => void) => {
        (util as any).getReduxLog().then((logData: any) => {
  ...
  private addFeedbackFile(file: IFeedbackFile) {
    ...
    if (size + file.size > FeedbackResponderDialog.MAX_ATTACHMENT_SIZE) {
      onShowError(
        "Attachment too big",
        "Sorry, the combined file size must not exceed 40MB",
        undefined,
        false,
      );
    } else {
...
export default withTranslation(["common", NAMESPACE])(
  connect(mapStateToProps, mapDispatchToProps)(FeedbackResponderDialog) as any,
