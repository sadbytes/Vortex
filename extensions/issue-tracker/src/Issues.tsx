...
class IssueList extends ComponentEx<IProps, IIssueListState> {
  ...
  public render(): JSX.Element {
    ...
    return (
      <Dashlet className="dashlet-issues" title={t("Your issues")}>
        ...
        />
        <div className="list-issues-container">{this.renderIssues(issues)}</div>
      </Dashlet>
    );
  }
  ...
  private renderIssue = (issueId: string, issue: IGithubIssueCache) => {
    ...
    const feedbackRequiredLabels = (issue.labels ?? []).filter((label) =>
      isFeedbackRequiredLabel(label),
    );
      ...
      <div key={issue.number.toString()} className="issue-item">
        ...
        <div className="issue-item-title">
          <OverlayTrigger trigger={["hover", "focus"]} overlay={popover}>
            <a data-issue={issue.number.toString()} onClick={this.openIssue}>
              {issue.title}
            </a>
          </OverlayTrigger>
        </div>
    ...
    const distinct = [].concat([
      ...new Set(
        sorted.map((id) => {
          ...
          return issues[targetNumber]?.number !== undefined ? targetNumber : id;
        }),
      ),
    ]);
  ...
  private requestIssue(issueId: string): Promise<IGithubIssue> {
    return requestFromApi(this.issueURL(issueId)).then((issue: IGithubIssue) =>
          ...
          .mapSeries(filteredRes, (issue) => {
            ...
            if (
              ...
              return this.requestIssue(issueId).then((issueDetails) => {
                const replyRequired =
                  issueDetails.labels.find((lbl) => isFeedbackRequiredLabel(lbl.name)) !==
                  undefined;
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslation(["issue-tracker", "common"])(IssueList as any)) as any;
