...
const _LINKS = {
  ...
  // tslint:disable-next-line: max-line-length
  UNMANAGED:
    "https://wiki.nexusmods.com/index.php/Importing_from_Nexus_Mod_Manager:_Things_to_consider#Unmanaged_Files",
  // tslint:disable-next-line: max-line-length
  FILE_CONFLICTS:
...
const MIN_DISK_SPACE_OFFSET = 500 * 1e6;
  ...
  try {
    ...
    return err.code !== "EPERM"
      ...
      : Promise.reject(
          new Error(
            t(
              "Vortex is unable to read/open one or more of " +
...
export async function getArchives(
  ...
  const modFileNames = new Set<string>(
    Object.keys(parsedMods).map((key) => parsedMods[key].modFilename),
  );
...
export function createModEntry(
  ...
  return fileChecksum(path.join(sourcePath, input)).then((md5) =>
    fs
...
export async function calculateModsCapacity(
  modList: IModEntry[],
  cb: (err: Error, mod: string) => void,
): Promise<ModsCapacityMap> {
...
export async function generateModEntries(
