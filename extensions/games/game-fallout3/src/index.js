...
const tools = [
  ...
  },
  {
    ...
    requiredFiles: ["fose_loader.exe", "Data/fallout3.esm"],
    relative: true,
    exclusive: true,
    defaultPrimary: true,
  },
...
async function findGame() {
  ...
  if (storeGames.length > 1)
    log(
      "debug",
      "Mutliple copies of Fallout 3 found",
      storeGames.map((s) => s.gameStoreId),
    );
      ...
      actions: [
        {
          ...
          action: (dismiss) => {
            ...
            api.showDialog(
              ...
              {
                bbcode:
                  "{{gameName}} has multiple language options when downloaded from {{storeName}}. [br][/br][br][/br]" +
...
function main(context) {
  context.registerGame({
    ...
    requiredFiles: ["Data/fallout3.esm"],
    environment: {
      SteamAPPId: "22300",
    },
    details: {
