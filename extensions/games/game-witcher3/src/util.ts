...
export async function getManagedModNames(
  api: types.IExtensionApi,
  mods: types.IMod[],
...
export async function getAllMods(api: types.IExtensionApi) {
  ...
  const managedMods = await getManagedModNames(
    api,
    enabledMods.map((key) => mods[key]),
  );
  return Promise.resolve({
...
export function isSettingsFile(filePath: string) {
  return [".settings", PART_SUFFIX].some(
    (ext) =>
...
export function suppressEventHandlers(api: types.IExtensionApi) {
  ...
  const state = api.getState();
  return state.session.notifications.notifications.some(
    (n) => n.id === ACTIVITY_ID_IMPORTING_LOADORDER,
  );
}
