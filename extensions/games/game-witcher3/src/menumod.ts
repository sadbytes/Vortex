...
async function getExistingCache(state, activeProfile) {
  ...
  try {
    const cacheData = await fs.readFileAsync(
...
function toFileMapKey(filePath) {
  return path.basename(filePath).toLowerCase().replace(PART_SUFFIX, "");
}
...
function populateCache(
  ...
  const enabledMods = affectedModIds
    .filter(
      (key) =>
        mods[key]?.installationPath !== undefined &&
...
export async function onDidDeploy(api, deployment, activeProfile) {
  ...
  const notInCache: Set<string> = new Set(
    docFiles.map((file) => file.source).filter((modId) => !inCache.has(modId)),
  );
...
async function createMenuMod(api, modName, profile) {
  const mod = {
    ...
    attributes: {
      ...
      description:
        "This mod is a collective merge of setting files required by any/all " +
...
const copyIniFile = (source: string, dest: string) =>
  fs
    ...
    try {
      ...
      if (
        ...
        api.showErrorNotification(
          "Failed to install menu mod",
          new util.DataInvalid(
            "Required setting files are missing - please run the game at least once and try again.",
          ),
          { allowReport: false },
        );
        return;
...
export async function ensureMenuMod(api, profile) {
  ...
  if (mod === undefined) {
    ...
    api.store.dispatch(
      actions.setModAttribute(profile.gameId, modName, "name", "Witcher 3 Menu Mod"),
    );

    api.store.dispatch(
      actions.setModAttribute(profile.gameId, modName, "type", "witcher3menumoddocuments"),
    );

    api.store.dispatch(
      actions.setModAttribute(profile.gameId, modName, "logicalFileName", "Witcher 3 Menu Mod"),
    );
