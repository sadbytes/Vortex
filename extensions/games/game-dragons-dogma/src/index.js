...
function migrate101(api, oldVersion) {
  ...
  const raiseDDDANotif = (migratedModsSuccess) => {
    const needReinstallMsg = api.translate(
      migratedModsSuccess
        ? "Mods for Dragon's Dogma may need to be reinstalled"
        : "Mods for Dragon's Dogma NEED to be reinstalled",
      { ns: I18N_NAMESPACE },
    );
    const pleaseNote = migratedModsSuccess
      ? api.translate(
          "Please note: if you are happy with your current mod setup, and have not noticed any issues, " +
      ...
      : api.translate(
          "Please note: Vortex had attempted to automatically migrate your mods to the new " +
      ...
      actions: [
        {
          ...
          action: () => {
            api.showDialog(
              ...
              {
                text: api.translate(
                  "It was raised to our attention that certain DDDA mods are not being " +
                  ...
                  { replace: { pleasenote: pleaseNote }, ns: I18N_NAMESPACE },
                ),
              },
              [{ label: "Close" }],
            );
...
function main(context) {
  ...
  context.registerInstaller("dddainvalidmod", 25, testIsInvalidMod, (...args) =>
    reportInvalidMod(context, ...args),
  );

  return true;
}

function reportInvalidMod(
  context,
  files,
  destinationPath,
  gameId,
  ...
  const invalidModDialog = () =>
    new Promise((resolve, reject) => {
      ...
      context.api.showDialog(
        ...
        {
          text:
            'The archive "{{archiveName}}" does not fit the expected packaging pattern ' +
