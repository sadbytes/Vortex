...
function requiresLauncher(gamePath) {
  return fs
    ...
    .then((files) =>
      files.find((file) => file.indexOf(STEAM_DLL) !== -1) !== undefined
...
function main(context) {
  Object.keys(KOTOR_GAMES).forEach((key) => {
    ...
    context.registerGame({
      ...
      requiresLauncher: game.id === "kotor2" ? requiresLauncher : undefined,
      logo: game.logo,
      executable: () => game.exec,
      requiredFiles: [game.exec],
      environment: {
...
function installRootContent(files, destinationPath, gameId, progressDelegate) {
  const modTypeInstr = {
    type: "setmodtype",
    value: "kotor-root",
  };

  const instructions = [modTypeInstr].concat(
...
function installTSLModContent(api) {
  api.showErrorNotification(
    "Invalid Mod",
    "The mod you're installing is meant to be installed/run with the TSLPatcher tool - Vortex cannot install this mod",
    { allowReport: false },
  );
...
function installTSLContent(api) {
  api.showErrorNotification(
    "Invalid Mod",
    "Vortex cannot install TSLPatcher as a mod - this is a separate utility application which should be installed and used separately.",
    { allowReport: false },
  );
  return Promise.reject(new util.ProcessCanceled("Invalid mod"));
}
