...
function findGame(mabGame) {
  ...
  try {
    ...
    return util.steam.findByName(name).then((game) => game.gamePath);
  }
}
...
function installModuleMod(files, moduleName) {
  ...
  const trimIndex = filtered
    .find((file) => path.basename(file).toLowerCase() === MAB_MODULE_FILE)
    .indexOf(MAB_MODULE_FILE);
