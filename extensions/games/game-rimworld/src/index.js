...
const GIT_FILES = [".gitignore", ".gitattributes"];
...
function requiresLauncher(gamePath) {
  return fs
    ...
    .then((files) =>
      files.find((file) => file.endsWith(STEAM_DLL)) !== undefined
...
function testSupportedSteamMod(api, files, gameId, archivePath) {
  ...
  if (aboutFiles.length === 0) {
    ...
    api.sendNotification({
      type: "warning",
      message:
        "The archive {{modName}} might not get installed correctly, please review the mod description and the staging folder for the mod.",
      replace: {
