    ...
    props.api.showDialog(
      ...
      "Corrupt load order file",
      {
        bbcode: props.api.translate(
          "The load order file is in a corrupt state or missing. " +
...
export async function serialize(
  ...
  const filteredLO = loadOrder.filter(
    (lo) => !INVALID_LO_MOD_TYPES.includes(props.mods?.[lo?.modId]?.type),
  );
...
export async function deserialize(context: types.IExtensionContext): Promise<LoadOrder> {
  ...
  try {
    ...
    const diff = enabledModIds.filter(
      (id) =>
        !INVALID_LO_MOD_TYPES.includes(mods[id]?.type) &&
