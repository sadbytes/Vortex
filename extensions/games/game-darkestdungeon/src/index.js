...
function installNoProject(files, destinationPath) {
  ...
  const findExternalHeroFiles = (dirStruct, portraitPath, destPath) => {
    ...
    const diff = onlyFiles.filter(
      (file) => dirStruct.find((dir) => file === dir.source) === undefined,
    );
  ...
  if (dirStructure.length === 0) {
    ...
    portraits.forEach((portrait) => {
      ...
      dirStructure = dirStructure.concat(
        findExternalHeroFiles(dirStructure, portraitDir, heroPath),
      );
      dirStructure = dirStructure.concat(
        heroFiles.map((file) => {
          return {
    ...
    const leftOver = onlyFiles.filter(
      (file) =>
