...
function writeResourceCfg(resourceBasePath) {
  ...
  return filterResourceCfg(resourcePath).then((filtered) => {
...
function enableModding() {
  ...
  return parser.read(filePath).then((ini) => {
...
function testMixed(files, gameId) {
  ...
  const trayFile = files.find((file) => {
    ...
    supportedTools: [
      {
        ...
        requiredFiles: ["game/bin/TS4.exe"],
        relative: true,
      },
    ],
    requiredFiles: ["game/bin/TS4_x64.exe"],
  });

  context.registerModType(
    "sims4mixed",
    25,
    (gameId) => gameId === "thesims4",
    getMixedPath,
