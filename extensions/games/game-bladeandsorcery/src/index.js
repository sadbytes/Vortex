...
async function ensureModType(discovery, api) {
  ...
  return invalidMods.length > 0
    ? purgeMods(discovery, api).then(() => {
        invalidMods.forEach((key) =>
          api.store.dispatch(actions.setModType(GAME_ID, key, targetModType)),
        );
  ...
  await util.walk(modsPath, async (iter, stats) => {
    ...
    if (stats.isDirectory() && !invalidNames.includes(modName)) {
      const hasManifest = await fs
        .statAsync(path.join(iter, MOD_MANIFEST))
...
async function preSort(context, items, direction, refreshType) {
  ...
  const toDisplayItem = (modName, modId) => ({
    ...
    name: modName,
    imgUrl:
      mods[modId]?.attributes?.pictureUrl !== undefined
  ...
  if (refreshType === "refresh") {
    ...
    try {
      ...
      const preSorted = [].concat(managedItems, externalItems).sort((a, b) => {
    ...
    const preSorted = []
      .concat(managedItems, externalItems)
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          {},
          t(
            "You can adjust the load order for Blade and Sorcery by dragging and dropping " +
              "mods up or down on this page.",
            { ns: I18N_NAMESPACE },
          ),
        ),
      ),
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          {},
          t(
            "The mods displayed in this page are valid mods, confirmed to be deployed inside the " +
              "game's mods folder. If a mod is missing, try deploying your mods - if it's still missing - it's not a valid mod!",
            { ns: I18N_NAMESPACE },
          ),
        ),
        React.createElement(
          ...
          {},
          t(
            "If you cannot see your manually added mod in this load order - click refresh and Vortex " +
              "should be able to pick it up as long as it has a valid manifest.json file.",
            { ns: I18N_NAMESPACE },
          ),
        ),
        React.createElement(
          ...
          {},
          t(
            "The load order file will only be picked up by the game in version 8.4 Beta 5 and above",
            { ns: I18N_NAMESPACE },
          ),
        ),
      ),
...
function requiresLauncher(gamePath) {
  ...
  return findGame()
    .then((steamPath) =>
      steamPath.toLowerCase() === gamePath.toLowerCase()
...
function main(context) {
  ...
  context.registerModType(
    ...
    getOfficialDestination,
    () => Promise.resolve(false),
    { name: "Official Mod (v1.0+)" },
  );
  ...
  context.registerModType(
    ...
    () => Promise.resolve(false),
    { name: "Legacy Mod (v0.8.3 and below)" },
  );
      ...
      return invalidMods.length > 0
        ? new Promise((resolve) => {
            context.api.showDialog(
              "error",
              "Blade & Sorcery version mismatch",
              {
                text:
                  "Vortex has detected a change in the installed version of Blade & Sorcery " +
