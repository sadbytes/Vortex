...
async function installOfficialMod(files, destinationPath, gameId, progressDelegate, api) {
      ...
      instructions: [
        {
          ...
          value: "fatal",
          source:
            "Blade & Sorcery is not discovered, please manage the game first and follow the instructions there.",
        },
      ],
    };
  }
  ...
  const manifestFiles = files.filter((file) => path.basename(file).toLowerCase() === MOD_MANIFEST);

  const createInstructions = (manifestFile) =>
    getModName(path.join(destinationPath, manifestFile), "Name", undefined).then(
      async (manifestModName) => {
        const isUsedModName =
          usedModNames.find((modName) => modName === manifestModName) !== undefined;
        const idx = isEngineInject
          ? manifestFile.indexOf("BladeAndSorcery_Data")
          : manifestFile.indexOf(path.basename(manifestFile));
        const rootPath = path.dirname(manifestFile);
        const modName = isUsedModName ? path.basename(rootPath) : manifestModName;

        usedModNames.push(modName);

        // Remove directories and anything that isn't in the rootPath (unless mod type engine inject).
        const filtered = files.filter(
          (file) => (file.indexOf(rootPath) !== -1 || isEngineInject) && !file.endsWith(path.sep),
        );
    ...
    actions: [
      {
        ...
        action: (dismiss) =>
          api.showDialog(
            ...
            {
              text: api.translate(
                "The mod you're attempting to install is not compatible with " +
