          ...
          actions: [
            {
              ...
              action: (dismiss) =>
                api.showDialog(
                  "info",
                  "Mods failed migration",
                  {
                    text: api.translate(
                      "As part of implementing the Load Order system for " +
...
function migrateMod020(api, mod) {
  ...
  let allEntries = [];
  return util
    .walk(modPath, (entries) => {
      allEntries = allEntries.concat(entries);
    })
    .then(() => {
      const manifestFiles = allEntries.filter(
      ...
      if (manifestFiles.length !== 1 || path.dirname(manifestFiles[0]) !== modPath) {
        ...
        return Promise.reject(
          new util.ProcessCanceled(`Cannot migrate: ${path.basename(modPath)}`),
        );
      }

      return getModName(manifestFiles[0], "Name").then((modName) => {
        const newPath = path.join(modPath, modName);
        const directories = allEntries
          .filter((entry) => path.extname(path.basename(entry)) === "")
          .map((entry) => ({
            src: entry,
