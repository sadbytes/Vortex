  ...
  return fs
    ...
    .then((files) => {
      ...
      return util.reduce(
        ...
        (modName, file) => {
          return fs
            .readFileAsync(path.join(indexPath, file))
            .then(async (data) => {
              if (modName !== "") {
...
async function installContent(files, destinationPath, gameId, progressDelegate) {
  ...
  const hasIndexFolder = await fs
    .statAsync(path.join(destinationPath, basePath, "index"))
  ...
  return fs
    .readFileAsync(contentFile, { encoding: "utf8" })
    .then(async (data) => {
      let parsed;
      try {
        parsed = await parseStringPromise(data);
        ...
        actions: [
          {
            ...
            action: () => {
              api.showDialog(
                ...
                {
                  text:
                    "Due to a bug in our X4 mod installer, some of your mods have been " +
